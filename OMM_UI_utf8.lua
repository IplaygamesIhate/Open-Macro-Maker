...
function UI.DrawWorkbenchWindow(ctx, env)
    ...
    if visible then
        ...
        if select(2, pcall(reaper.ImGui_Button, ctx, "Γûá DESIGN", btn_w, btn_h)) then 
        ...
        if select(2, pcall(reaper.ImGui_Button, ctx, "Γû╢ PLAY", btn_w, btn_h)) then UI.edit_mode = false end
        pcall(reaper.ImGui_PopStyleColor, ctx, 2)
